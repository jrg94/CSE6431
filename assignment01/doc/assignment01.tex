...
\begin{homeworkProblem}
    ...
    attempts to acquire the sr_mutex. If successful, it verifies that there
    aren't any writers writing or readers reading. If there are, the process
    increments the writers_waiting variable. Otherwise, the process gives the
    go ahead for writing and increments the writers_writing variable.

    At that point, the process gives up the sr_mutex and attempts to acquire
    the writer_mutex to begin writing. If successful, the process begins
    writing before attempting to acquire the sr_mutex once again.
    ...
    to the next reader before freeing the sr_mutex.
