...
\begin{homeworkProblem}
    ...
    Consider the following graphs containing three transactions, A, B, and C.
    Each graph contains a list of nodes followed by a list of nodes they point
    to:
    ...
    The first graph generates one possible topological sort:

    \begin{enumerate}
      \item A -> C -> B
    \end{enumerate}

    The second graph generates two possible topological sort:
    ...
    Notice how both graphs share one topological sort. Therefore, it's possible
    that two equivalent logs have two different serialization graphs.
